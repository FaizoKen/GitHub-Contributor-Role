...
//!
//! Skips `oauth_states` entirely. Unlike the Genshin migration, the reason
//! here is different: this plugin's `oauth_states` is still in active use
//! by its own GitHub OAuth flow and must remain in the local DB.
//!
//! Usage (PowerShell):
//!   $env:DATABASE_URL="postgres://gh:password@localhost:5432/github_roles"
...
async fn main() {
    ...
    let plugin_url = env::var("DATABASE_URL").expect("DATABASE_URL (GitHub plugin DB) must be set");
    let gateway_url = env::var("GATEWAY_DATABASE_URL")
        .expect("GATEWAY_DATABASE_URL (Auth Gateway DB) must be set");

    println!("Connecting to source (GitHub plugin)...");
    let src = PgPoolOptions::new()
        .max_connections(4)
        .acquire_timeout(std::time::Duration::from_secs(10))
        .connect(&plugin_url)
        .await
        .expect("failed to connect to GitHub plugin DB");
    ...
    println!(
        "\nDone. oauth_states was intentionally skipped — \
         it stays in the GitHub plugin DB for the GitHub OAuth flow."
    );
}
...
async fn migrate_discord_tokens(src: &PgPool, dst: &PgPool) {
    ...
    // a freshly-bootstrapped plugin DB, or after migration 006 has dropped it).
