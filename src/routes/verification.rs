...
const SESSION_COOKIE: &str = "gh_session";
...
pub async fn login(State(state): State<Arc<AppState>>) -> Response {
    ...
    if let Err(e) = sqlx::query("INSERT INTO oauth_states (state, expires_at) VALUES ($1, $2)")
        .bind(&state_param)
        .bind(expires)
        .execute(&state.pool)
        .await
...
pub async fn github_login(
    ...
    let url = discord_oauth::github_authorize_url(&state.config, &state_param);
...
pub async fn github_callback(
    ...
    let (github_username, github_id) =
        discord_oauth::github_exchange_code(&state.oauth_http, &state.config, &code).await?;
